...
export type DictionaryResult = {
  ...
  theological_distinctions: Array<{
    title: string;
    text: string;
  }>;
...
- Não cite autores modernos nem bibliografia externa; foque em Bíblia e teologia sistemática (sem nomes).
- Evite generalidades: cada seção precisa de conteúdo objetivo.
...
- Termos originais relevantes quando aplicável.
- Textos-chave: mínimo 5 (com referência, trecho curto e explicação).
- Textos de suporte: mínimo 8 (referência + explicação).
- Distinções teológicas: mínimo 5.
- Termos relacionados: mínimo 8.
...
function apiUrl(path: string) {
  // Web: relativo para bater no mesmo host
  if (Platform.OS === 'web') return path.startsWith('/') ? path : `/${path}`;
  // Mobile: absoluto
...
export async function fetchAIAnalysis(
  ...
  try {
    const systemPrompt =
      mode === 'DICTIONARY' ? DICTIONARY_SYSTEM :
      mode === 'VERSE' ? VERSE_SYSTEM :
      CHAPTER_SYSTEM;

    const userPrompt = buildUserPrompt(text, mode);

    // Endpoint do seu Vercel (NÃO OpenAI direto)
    const url = apiUrl('/api/chat');

    const res = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      // formato compatível com o que você já usa no ReadBookScreen
      body: JSON.stringify({
        messages: [
    ...
    Alert.alert('Erro', error?.message || 'Falha na conexão com Dr. Logos.');
    return null;
  }
}

/**
 * Espera que seu backend retorne { base64: "..." } ou { audioBase64: "..." }
 * (ajusto fácil se você colar seu /api/tts ou função do Vercel)
 */
