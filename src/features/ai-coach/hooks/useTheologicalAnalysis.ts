...
export function useTheologicalAnalysis() {
  ...
  const analyzeContext = async (prompt: string, contextTitle: string) => {
    ...
    try {
      ...
      console.error(error);
      if (mountedRef.current) setData({ theme: 'Erro', exegesis: 'Falha na comunicação.' });
    } finally {
      if (mountedRef.current) setLoading(false);
    }
  };

  // 2) Falar (iOS/Android: grava no cache e toca via arquivo local)
